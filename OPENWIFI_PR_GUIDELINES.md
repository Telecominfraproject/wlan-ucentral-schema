...
Merge commits are reserved exclusively for the LTS branch, `main-v<X.Y.0>-LTS` (see B7).
...
> `main`.** A fix for an already-shipped release (4.2.x) is a **backport PR that
> targets this repo's `main-v<X.Y.0>-LTS` branch** (currently `main-v4.2.0-LTS`) —
> the `release/vX.Y.0` branches live in `wlan-ap`, not here (see B7) — see
> [OPENWIFI_RELEASE_PROCEDURE.md](https://github.com/Telecominfraproject/wlan-ap/blob/main/OPENWIFI_RELEASE_PROCEDURE.md)
> in `wlan-ap`, which drives releases for this repo as a submodule.
...
`main` (release backports branch off `main-v<X.Y.0>-LTS` instead; see the release
procedure).
...
- `main` is the trunk — **feature/general-fix PRs land there first**. Changes for an LTS
  release go onto its **`main-v<X.Y.0>-LTS`** branch (currently `main-v4.2.0-LTS`), where
  they are validated.
- **This repo carries no `release/*` branches and no version tags.** It is a submodule of
  `wlan-ap`, so a schema change reaches a release when `wlan-ap` bumps the submodule on its
  own release branch and tags there (see B5 for the bump format, and the release procedure
  for the tagging flow). Nothing here is tagged `vX.Y.Z`.
- Unlike `main`, the LTS branch is **merge-based**: backport PRs are merged in rather than
  rebased, e.g. `Merge pull request #76 from .../WIFI-15031-fix-fingerprinting-raw-mode`.
  Don't rebase `main-v<X.Y.0>-LTS`.
- **Trunk-first (required):** a fix that also applies to the trunk **lands on `main` first**,
  then is **cherry-picked** into the LTS branch (`main-v<X.Y.0>-LTS`). Do **not** open
  ...
  [OPENWIFI_RELEASE_PROCEDURE.md](https://github.com/Telecominfraproject/wlan-ap/blob/main/OPENWIFI_RELEASE_PROCEDURE.md)
  in `wlan-ap`; this repo has no release branches of its own and ships via the
  `wlan-ap` submodule bump.
...
- [ ] LTS work merged only on `main-v<X.Y.0>-LTS`; `main` stays linear and untagged.
