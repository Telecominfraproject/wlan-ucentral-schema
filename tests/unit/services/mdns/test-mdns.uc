...
function run_test(test_name, input_file, expected_file) {
	...
	try {
		...
		// Render template
		let output = render("../../../../renderer/templates/services/mdns.uc", context);
...
function main() {
	printf("=== MDNS Service Template Tests ===\n\n");
	
	// Run all test cases
	run_test("mdns-basic", "input/mdns-basic.json", "output/mdns-basic.uci");
	run_test("mdns-no-interfaces", "input/mdns-no-interfaces.json", "output/mdns-no-interfaces.uci");
	run_test("mdns-single-interface", "input/mdns-single-interface.json", "output/mdns-single-interface.uci");
	run_test("mdns-fingerprint", "input/mdns-fingerprint.json", "output/mdns-fingerprint.uci");
	...
	printf("All MDNS service tests passed!\n");
}
