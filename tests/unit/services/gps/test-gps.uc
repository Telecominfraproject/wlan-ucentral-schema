...
function run_test(test_name, input_file, expected_file) {
	...
	try {
		...
		// Render template
		let output = render("../../../../renderer/templates/services/gps.uc", context);
...
function main() {
	printf("=== GPS Service Template Tests ===\n\n");
	
	// Run all test cases
	run_test("gps-basic", "input/gps-basic.json", "output/gps-basic.uci");
	run_test("gps-custom", "input/gps-custom.json", "output/gps-custom.uci");
	run_test("gps-disabled", "input/gps-disabled.json", "output/gps-disabled.uci");
	...
	printf("All GPS service tests passed!\n");
}
