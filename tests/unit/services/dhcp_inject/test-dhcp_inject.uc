...
function run_test(test_name, input_file, expected_file) {
	...
	try {
		...
		// Render template
		let output = render("../../../../renderer/templates/services/dhcp_inject.uc", context);
...
function main() {
	printf("=== DHCP Inject Service Template Tests ===\n\n");
	
	// Run all test cases
	run_test("dhcp_inject-basic", "input/dhcp_inject-basic.json", "output/dhcp_inject-basic.uci");
	run_test("dhcp_inject-no-config", "input/dhcp_inject-no-config.json", "output/dhcp_inject-no-config.uci");
	run_test("dhcp_inject-no-ssids", "input/dhcp_inject-no-ssids.json", "output/dhcp_inject-no-ssids.uci");
	run_test("dhcp_inject-default-ports", "input/dhcp_inject-default-ports.json", "output/dhcp_inject-default-ports.uci");
	...
	printf("All DHCP Inject service tests passed!\n");
}
