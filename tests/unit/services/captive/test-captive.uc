...
function run_test(test_name, input_file, expected_file) {
	...
	try {
		...
		// Render template
		let output = render("../../../../renderer/templates/services/captive.uc", context);
...
function main() {
	printf("=== Captive Service Template Tests ===\n\n");
	
	// Run all test cases
	run_test("captive-basic", "input/captive-basic.json", "output/captive-basic.uci");
	run_test("captive-credentials", "input/captive-credentials.json", "output/captive-credentials.uci");
	run_test("captive-radius", "input/captive-radius.json", "output/captive-radius.uci");
	run_test("captive-no-service", "input/captive-no-service.json", "output/captive-no-service.uci");
	run_test("captive-no-ssids", "input/captive-no-ssids.json", "output/captive-no-ssids.uci");
	run_test("captive-multiple-interfaces", "input/captive-multiple-interfaces.json", "output/captive-multiple-interfaces.uci");
	run_test("captive-upstream", "input/captive-upstream.json", "output/captive-upstream.uci");
	...
	printf("All Captive service tests passed!\n");
}
