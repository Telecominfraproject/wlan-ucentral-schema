...
function run_test(test_name, input_file, expected_file) {
	...
	try {
		...
		// Render template
		let output = render("../../../../renderer/templates/services/lldp.uc", context);
...
function main() {
	printf("=== LLDP Service Template Tests ===\n\n");
	
	// Run all test cases
	run_test("lldp-basic", "input/lldp-basic.json", "output/lldp-basic.uci");
	run_test("lldp-no-service", "input/lldp-no-service.json", "output/lldp-no-service.uci");
	run_test("lldp-multiple-interfaces", "input/lldp-multiple-interfaces.json", "output/lldp-multiple-interfaces.uci");
	...
	printf("All LLDP service tests passed!\n");
}
