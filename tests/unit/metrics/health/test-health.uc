...
function run_test(test_name, input_file, expected_file) {
	...
	try {
		...
		// Render template
		let output = render("../../../../renderer/templates/metric/health.uc", context);
...
function main() {
	printf("=== Health Metrics Template Tests ===\n\n");
	
	// Run all test cases
	run_test("health-basic", "input/health-basic.json", "output/health-basic.uci");
	run_test("health-no-config", "input/health-no-config.json", "output/health-no-config.uci");
	run_test("health-all-disabled", "input/health-all-disabled.json", "output/health-all-disabled.uci");
	run_test("health-selective", "input/health-selective.json", "output/health-selective.uci");
	run_test("health-custom-interval", "input/health-custom-interval.json", "output/health-custom-interval.uci");
	...
	printf("All health metrics tests passed!\n");
}
