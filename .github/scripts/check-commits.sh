...
if [ -n "$HEAD_REF" ] && ! echo "$HEAD_REF" | grep -qE '^staging-WIFI-[0-9]+-[a-zA-Z0-9.-]+$'; then
  ...
  if [ -n "$branch_ticket" ]; then
    warn "Branch \`$HEAD_REF\` is missing the \`-<short-hyphenated-description>\` suffix; expected \`staging-$branch_ticket-<short-hyphenated-description>\`, e.g. \`staging-$branch_ticket-renderer-skip-160mhz-fallback\` (guidelines A1)."
  else
...
# rejected subject should say *what* is wrong ("renderer / templates:" has
# spaces), not just "not canonical".
...
# around separators that causes almost every rejection ("renderer / templates",
# "schema "). Prints nothing when the result is still not a valid token.
suggest_prefix() {
  local p
  ...
  if echo "$subject" | grep -qE '^Revert "'; then
    ...
    if [ -z "$prefix_chain" ]; then
      ...
      if [[ "$subject" =~ ^[A-Za-z0-9._/-]+:[[:space:]]*$ ]]; then
        err "$short: subject \`$subject\` is only a subsystem prefix with no summary. Add an imperative summary, e.g. \`${subject%%:*}: add CUI to ACL RADIUS Access-Request\` (guidelines A3)."
        ...
        err "$short: subject \`$(trunc "$subject")\` has no subsystem prefix. Use \`subsystem: imperative summary\`, e.g. \`renderer: skip 160MHz fallback when DFS is disabled\` (guidelines A3)."
        ...
        err "$short: subject starts with the ticket \`${bad_prefix%% *}\`. The ticket belongs in a \`Fixes:\` trailer, not the subject: \`renderer: skip 160MHz fallback when DFS is disabled\`, not \`WIFI-14728 renderer : skip 160mhz fallback\` (guidelines A3)."
      else
        err "$short: invalid subsystem prefix \`$bad_prefix:\` — only letters, digits and \`. _ / -\` are allowed, e.g. \`ucentral-schema:\`, \`patches-25.12:\`, \`qca-wifi-7: wifi-scripts:\` (guidelines A3)."
      fi
    elif [[ "$first_prefix" =~ $TICKET_RE ]]; then
      err "$short: subsystem prefix is the ticket \`$first_prefix\`. Name the subsystem instead and keep the ticket in a \`Fixes:\` trailer, e.g. \`renderer: skip 160MHz fallback when DFS is disabled\` (guidelines A3)."
    elif [ -z "$summary" ]; then
      err "$short: subject is only the prefix \`$prefix_chain\` with no summary. Add an imperative summary, e.g. \`${first_prefix}: add CUI to ACL RADIUS Access-Request\` (guidelines A3)."
