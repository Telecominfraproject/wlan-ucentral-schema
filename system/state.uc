...
cursor.foreach("network", "interface", function(d) {
	...
	let status_v4 = ctx.call(sprintf("network.interface.%s_4", name), "status");
	let status_v6 = ctx.call(sprintf("network.interface.%s_6", name), "status");
	...
	if (index(stats.types, 'ssids') >= 0 && length(wifistatus)) {
		...
		for (let radio, data in wifistatus) {
			for (let k, vap in data.interfaces) {
				...
				// frequencies (same as the radio path) so the controller filters
				// them into the correct band.
				if (index(ssid.band, 'S1G') != -1 || index(ssid.band, 'HALOW') != -1) {
					let mapping_table = map5GToS1G();
